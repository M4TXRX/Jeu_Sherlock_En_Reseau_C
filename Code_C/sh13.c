...
void *fn_serveur_tcp(void *arg)
{
        ...
        portno = gClientPort;
        serv_addr.sin_family = AF_INET;						//protocole IPV4		
        serv_addr.sin_addr.s_addr = INADDR_ANY;					//nimporte quelle IP acceptéE
        serv_addr.sin_port = htons(portno);					//portno est le port connus
       if (bind(sockfd, (struct sockaddr *) &serv_addr, sizeof(serv_addr)) < 0)
        ...
        while (1) //on tourne dans l'attente d'un message réseau
        {
                newsockfd = accept(sockfd, (struct sockaddr *) &cli_addr, &clilen); //fait une copie de lui même et donne le travail à un de ses fils
                if (newsockfd < 0)
                {
                        printf("accept error\n");
                        exit(1);
                ...
                bzero(gbuffer,256);
                n = read(newsockfd,gbuffer,255); //appel système standard de unix pour lire ,un socket est un descripteur de fichier
                if (n < 0)
                {
                        printf("read error\n");
                        exit(1);
                ...
                synchro=1; //j'ai reçu un message réseau
...
int main(int argc, char ** argv)
{
	...
	char winner[1][256];
        ...
        strcpy(gName,argv[5]);
		strcat(TitreFenetre,gName); //On renomme la fenetre avec le nom du joueur

    SDL_Init(SDL_INIT_VIDEO);
	TTF_Init();
 
    SDL_Window * window = SDL_CreateWindow(TitreFenetre,
   ...
   ret = pthread_create ( & thread_serveur_tcp_id, NULL, fn_serveur_tcp, NULL);
		...
		switch (gbuffer[0])
		  {
			...
			case 'M':
				// RAJOUTER DU CODE ICI
				sscanf(gbuffer,"%c %d", &temp, &OKJoueurCourant);
     
				
				if(OKJoueurCourant == gId)
                {
					goEnabled = 1;
				}
				else
			...
			case 'V':
				// RAJOUTER DU CODE ICI
				sscanf(gbuffer,"%c %d %d %d", &temp, &ligne, &colonne, &MemTemp);								
				...
				{	
					...
					tableCartes[ligne][colonne] = MemTemp;
					cpt1++;
					}
				}
				...
				//le jeu s arrete
				case 'F'://tous les joueurs ont perdu
				printf("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% \n\nDommage tous les joueurs ont perdu ! Fin de la partie.\n\n%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% \n\n");
				quit = 1;

				break;
				
				//Il y a un gagnant le jeu s arrete
				case 'W' :

				sscanf(gbuffer,"%c %s", &temp, winner[0]);								
				printf("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% \n\n Joueur %s a gagné !\n%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% \n\n",winner[0]);
				quit = 1;
				
				break;
				
				//Un joueur s'est trompé et a perdu
				case 'P' : 

				sscanf(gbuffer,"%c %s", &temp, perdant[0]);								
				printf("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% \n\n Joueur %s tu as perdu bien essayé !\n%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% \n\n",perdant[0]);
				//quit = 1;
	...
	if (connectEnabled==1)
	{
        ...
        //SDL_SetRenderDrawColor(rendsynchroerer, 255, 0, 0, SDL_ALPHA_OPAQUE);
